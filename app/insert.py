...
def product(product_name, manufacturer_name, price, db, cursor):
    '''
    ...
    >>> product("Panadol - 26 pill box", "Chemical Company", 5.60, connect, mycursor)
    ...
    alist = read.check_value("product", "product_id",
                                "product_name", product_name, cursor)
    if len(alist) == 0:
        manufacturer_id = manufacturer(manufacturer_name, db, cursor)
        query = (
...
def manufacturer(manufacturer_name, db, cursor):
    '''
    ...
    alist = read.check_value("manufacturer", "manufacturer_id",
...
def batch(product_id, exp_date, import_date, quantity, db, cursor):
    ...
    >>> batch(4, '2021-12-10', '2019-09-10', 500, connect, mycursor)
    @return: 6
...
def inventory(batch_id, product_id, quantity, db, cursor):
    '''
    ...
    >>> inventory(5, 2, 50, connect, mycursor)
    @return: 6
    '''
...
def customer(surname, given_name, address, dob, db, cursor):
    '''
    ...
    Example:
    >>> customer('Matthew', 'Peterson', 'vic 3033', '1998-08-08', connect, mycursor)
